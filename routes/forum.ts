...
// Layout wrapper
const layout = (content: string, session: any) => {
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .submolt-item { padding: 0.75rem; border-radius: 4px; cursor: pointer; transition: background 0.2s; display: block; text-decoration: none; color: inherit; }
    ...
    .post-body { margin-top: 0.5rem; color: #ccc; line-height: 1.5; white-space: pre-wrap; }
...
forum.get('/', async (c) => {
  ...
  const submoltsList = submolts.map(s => 
    `<a href="/forum?m=${s.name}" class="submolt-item ${submoltFilter === s.name ? 'active' : ''}">
  ...
  const createBtn = session ? '<a href="/forum/submit" class="btn">Create Post</a>' : '';
  const currentTitle = submoltFilter 
    ? submolts.find(s => s.name === submoltFilter)?.display_name || 'All Posts'
    : 'All Posts';
    ...
    <div class="layout">
      <div class="sidebar">
        <div class="submolt-list">
          ...
          <a href="/forum" class="submolt-item ${!submoltFilter ? 'active' : ''}">
      ...
      <div class="main">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
          <h1 style="font-size: 2rem;">${currentTitle}</h1>
          ${createBtn}
        </div>
...
forum.get('/submit', async (c) => {
  ...
  const submoltOptions = submolts.map(s => 
